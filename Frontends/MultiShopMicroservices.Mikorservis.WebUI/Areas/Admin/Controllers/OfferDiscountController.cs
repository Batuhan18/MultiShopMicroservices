...
namespace MultiShopMicroservices.Mikorservis.WebUI.Areas.Admin.Controllers
{
    ...
    public class OfferDiscountController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("http://localhost:7110/api/OfferDiscounts");
        ...
        public async Task<IActionResult> CreateOfferDiscount(CreateOfferDiscountDto createOfferDiscountDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:7110/api/OfferDiscounts", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("http://localhost:7110/api/OfferDiscounts/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "OfferDiscount", new { area = "Admin" });
            }
        ...
        public async Task<IActionResult> UpdateOfferDiscount(string id)
        {
            ...
            ViewBag.v0 = "İndirim Teklifler İşlemleri";
            var client = _httpClientFactory.CreateClient();
            var responseMessgae = await client.GetAsync("http://localhost:7110/api/OfferDiscounts/" + id);
        ...
        public async Task<IActionResult> UpdateOfferDiscount(UpdateOfferDiscountDto updateOfferDiscountDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessgae = await client.PutAsync("http://localhost:7110/api/OfferDiscounts/", stringContent);
            if (responseMessgae.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "OfferDiscount", new { area = "Admin" });
            }
